...
def run(
    ...
    for i, (name, f, suffix, gpu) in convert_mobile.export_formats().iterrows():  # index, (name, file, suffix, gpu-capable)
        try:
            ...
            if f == '-':
                ...
                w = convert_mobile.run(weights=weights, imgsz=[imgsz], include=[f], device=device, half=half)[-1]  # all others
...
def test(
    ...
    for i, (name, f, suffix, gpu) in convert_mobile.export_formats().iterrows():  # index, (name, file, suffix, gpu-capable)
        try:
            w = weights if f == '-' else \
                convert_mobile.run(weights=weights, imgsz=[imgsz], include=[f], device=device, half=half)[-1]  # weights
            assert suffix in str(w), 'export failed'
            y.append([name, True])
        except Exception:
