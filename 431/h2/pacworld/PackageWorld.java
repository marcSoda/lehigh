...
public class PackageWorld extends Environment{
   ...
   /** The penalty per turn per package delivered */
   public static int EFFORT_PENALTY = 4;
   ...
   // allow extra time before terminating
   protected static boolean patientTerminate = false;
   ...
   protected boolean isComplete() {
      ...
      /* run for at least 100 turns if patientTerminate = true */
      if (s.getNumPackages() > 0 && patientTerminate == true) {
    	  if (count < getNumAgents() * 500)
    		  return false;
      }
      ...
      if (s.getNumPackages() == 0 || 
    	   ...
    	   * (count*s.getOrigNumPackages()*EFFORT_PENALTY)/(s.getOrigNumPackages()-s.getNumPackages()) > 1000
    	   * If this is true, the agents score will go below 0 if they continue to deliver packages at
    	   * the current rate. However, this would not account for the possibility that the agents are
    	   * in a planning phase and delivery will significantly pick up.
    	   * 
    	   *  An alternative to using current effort to predict the future rate is to estimate the
    	   *  minimum time to deliver. We could assume that on average, the x distance between a
    	   *  package and its destination is worldSize/3, and likewise for the y-distance. Thus,
    	   *  the average distance is worldSize*2/3. And the formula would be:
    	   *  (count + s.getNumPackages()*worldSize*2/3)*EFFORT_PENALTY/s.getOrigNumPackages() > 1000
    	   */
      {
         //s.printState();
   ...
   public int getTeamPerformanceMeasure() {
      ...
      int perf = (int)(((float)delivered / s.getOrigNumPackages() * 1000) - 
            (avgEffort * EFFORT_PENALTY) - (msgLengthPerTurn / 50) - thinkPerTurn);
      return perf;
   }
